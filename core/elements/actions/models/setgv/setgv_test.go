...
func (suite *ActionTestSuite) TestSetGlobalVariable() {
	assert := assert.New(suite.T())

	test.CheckAFields(suite.T(), SetGlobalVariable)
	...
	args := [][]data.UserArg{
		...
		[]data.UserArg{
			data.UserArg{
				ID:      SetGlobalVariable.Args[0].ID,
				Content: suite.VariableName,
			},
			data.UserArg{
				ID:      SetGlobalVariable.Args[1].ID,
		...
		[]data.UserArg{
			...
			data.UserArg{
				ID:      SetGlobalVariable.ID + "-5", // Non-existent ID
				Content: suite.VariableContent,
			},
		},
	...
	for i, arg := range args[:3] {
		...
		r, cr, err := SetGlobalVariable.Run(&shared.ChainedResult{}, &ua, suite.TaskID)
		...
		assert.FileExistsf(filepath.Join(suite.TestDir, arg[0].Content), "the variable should be saved on a file (arg %d)", i)
	}

	// Remove the file of the default variable to avoid a false positive.
	_, err := os.Stat(filepath.Join(suite.TestDir, suite.VariableName))
	if !os.IsNotExist(err) {
		err := os.Remove(filepath.Join(suite.TestDir, suite.VariableName))
		if err != nil {
			panic(err)
		}
	}
	...
	for i, arg := range args[3:] {
		...
		assert.NoFileExistsf(filepath.Join(suite.TestDir, arg[0].Content), "the variable shouldn't be saved on a file because the execution must fail (arg %d)", i)
	}
}
